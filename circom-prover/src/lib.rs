//! FeliCa DES oracle prover, circom-compatible mirror (spec `docs/spec.md` §7).
//!
//! API-compatible with `felica-prover`: same [`ProveRequest`],
//! [`Attestation`], [`Groth16Proof`], [`ProverError`], [`prove`],
//! [`verify_attestation`], [`commit`], [`blank_constraint_counts`], and the
//! same `circuit::` / `des::` items, so the `prover` test suite runs here by
//! renaming the import. The proving backend is still Groth16 over BN254
//! (`ark-groth16`), keeping proving speed on par with `felica-prover`.
//!
//! What differs is readability and tooling compatibility: the constraint
//! system is organized as a 1:1 mirror of the circom sources in
//! `circuits/` (`des.circom`, `felica.circom`), each Rust helper citing its
//! circom template. The `.circom` files are the human-readable spec of the
//! same relations and compile with `circom` 2.x for the snarkjs / Solidity /
//! Move toolchains (`scripts/compile.sh`), while `cargo test` exercises this
//! Rust mirror (no Node dependency).
//!
...
//!
//! NOTE: each crate (`felica-prover`, `felica-circom-prover`) runs its own
//! test-only setup, so proving/verification keys differ between crates.
//! Proofs are NOT interchangeable across crates; verifiers must pin the `vk`
//! of the backend they accept. Public inputs, hex encodings, and error
//! mappings are identical, so swapping backends is a one-line change.
