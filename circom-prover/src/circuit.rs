//! R1CS mirror of the circom circuits (`circuits/des.circom`,
//! `circuits/felica.circom`) enforcing spec §7 constraints 1–6.
//!
...
//!
//! Circom correspondence (template → Rust item, same signal flow):
//! - `SBox` → [`sbox_gadget`], `DesBlock` → [`des_gadget`],
//!   `TdesEde` → [`tdes_encrypt_gadget`]
//! - `CbcDecryptAuth2` → [`cbc_decrypt_blocks`], `MacVerify13` → [`mac_verify`]
//! - `PackPublicInputs` → [`constrain_public_packing`],
//!   `CheckChallenges` → [`constrain_challenges`]
//! Constraint semantics (tables, S-box polys, 8-limb packing) are identical
//! to `felica-prover`; only the organization follows the circom hierarchy so
//! the `.circom` sources stay auditable against this executable mirror.
...
/// S-box gadget (circom `SBox`): 6 input bits (b0 first/MSB) → 4 output
/// bits (o0 first/MSB). Output witnesses carry native values for
/// `witness_in`; constraints pin them via `out == P(in)`.
pub fn sbox_gadget(
    cs: ConstraintSystemRef<Fr>,
    box_idx: usize,
    input: &[Boolean<Fr>],
...
/// Single DES block gadget (circom `DesBlock`). `data_des` is 64
/// DES-ordered bits, `key_le` is 64 LE key bits, `encrypt` selects the round
/// key order at witness-build time (circom `enc` template parameter — never
/// a signal), `witness_data`/`witness_key` are native bytes for hints.
...
/// 3DES-EDE encrypt gadget (circom `TdesEde`): E(k1)→D(k2)→E(k1).
#[allow(clippy::too_many_arguments)]
...
/// Allocated bit witnesses plus the 8 public-input variables.
struct SessionWires {
    r1_le: Vec<Boolean<Fr>>,
    c1b_le: Vec<Boolean<Fr>>,
    c2a_le: Vec<Boolean<Fr>>,
    auth2_le: Vec<Boolean<Fr>>,
    idi_le: Vec<Boolean<Fr>>,
    r2_le: Vec<Boolean<Fr>>,
    l_le: Vec<Boolean<Fr>>,
    beta_le: Vec<Boolean<Fr>>,
}

/// Allocate LE-bit witnesses and link them to the 8 packed public inputs
/// (circom `PackPublicInputs`). Returns the wires; the public-input
/// variables are created inside the constraint system as instance variables.
fn constrain_public_packing(
    cs: ConstraintSystemRef<Fr>,
    c: &FelicaCircuit,
) -> Result<SessionWires, SynthesisError> {
    // --- witnesses (LE bits) ---
    let wires = SessionWires {
        r1_le: alloc_le_bits(cs.clone(), &c.r1)?,
        c1b_le: alloc_le_bits(cs.clone(), &c.c1b)?,
        c2a_le: alloc_le_bits(cs.clone(), &c.c2a)?,
        auth2_le: alloc_le_bits(cs.clone(), &c.auth2)?,
        idi_le: alloc_le_bits(cs.clone(), &c.idi)?,
        r2_le: alloc_le_bits(cs.clone(), &c.r2)?,
        l_le: alloc_le_bits(cs.clone(), &c.l)?,
        beta_le: alloc_le_bits(cs.clone(), &c.beta)?,
    };

    // --- public inputs (8 Fr) ---
    let pis = public_inputs_fr(
        &c.r1, &c.c1b, &c.c2a, &c.auth2, &c.cm, &c.idi, &c.r2, c.attested_at,
    );
    ...
    Boolean::le_bits_to_fp(&wires.r1_le)?.enforce_equal(&pi_vars[0])?;
    Boolean::le_bits_to_fp(&wires.c1b_le)?.enforce_equal(&pi_vars[1])?;
    Boolean::le_bits_to_fp(&wires.c2a_le)?.enforce_equal(&pi_vars[2])?;
    Boolean::le_bits_to_fp(&wires.auth2_le[..128])?.enforce_equal(&pi_vars[3])?;
    Boolean::le_bits_to_fp(&wires.auth2_le[128..])?.enforce_equal(&pi_vars[4])?;
    ...
    // cm_out (constraint 7 by construction), attested_at is echoed.
    {
        ...
        both.extend_from_slice(&wires.idi_le);
        both.extend_from_slice(&wires.r2_le);
        Boolean::le_bits_to_fp(&both)?.enforce_equal(&pi_vars[6])?;
    }
    Ok(wires)
}

/// Constraints 1–2 (circom `CheckChallenges`): `3DES(l,β,r1) == c1b` and
/// `3DES(l,β,r2) == c2a` (the latter ≡ `3DES⁻¹(l,β,c2a) == r2`).
fn constrain_challenges(
    cs: ConstraintSystemRef<Fr>,
    c: &FelicaCircuit,
    w: &SessionWires,
) -> Result<(), SynthesisError> {
    // --- DES-ordered views (wiring) ---
    let r1_des = le_to_des(&w.r1_le);
    let c1b_des = le_to_des(&w.c1b_le);
    let c2a_des = le_to_des(&w.c2a_le);
    let r2_des = le_to_des(&w.r2_le);

    // Constraint 1: 3DES(l,β,r1) == c1b.
    let e1 = tdes_encrypt_gadget(cs.clone(), &r1_des, &w.l_le, &w.beta_le, &c.r1, &c.l, &c.beta)?;
    for (a, b) in e1.iter().zip(c1b_des.iter()) {
        a.enforce_equal(b)?;
    }
    // Constraint 2: 3DES(l,β,r2) == c2a (≡ 3DES⁻¹(l,β,c2a) == r2).
    let e2 = tdes_encrypt_gadget(cs.clone(), &r2_des, &w.l_le, &w.beta_le, &c.r2, &c.l, &c.beta)?;
...
/// Constraint 3 (circom `CbcDecryptAuth2`): `p = DES-CBC-decrypt(r2,
/// auth2)`, zero IV. Returns the 256 plaintext LE bits plus the native
/// plaintext bytes (witness hints for [`mac_verify`]).
fn cbc_decrypt_blocks(
    cs: ConstraintSystemRef<Fr>,
    c: &FelicaCircuit,
    w: &SessionWires,
) -> Result<(Vec<Boolean<Fr>>, Vec<u8>), SynthesisError> {
    use crate::des::des_decrypt;
    let mut pt_des_blocks: Vec<Vec<Boolean<Fr>>> = Vec::with_capacity(4);
    // Native plaintext for hints.
    let pt_native: Vec<u8> = {
        let mut out = Vec::with_capacity(32);
        let mut prev = [0u8; 8];
        for chunk in c.auth2.chunks(8) {
            let ct: [u8; 8] = chunk.try_into().unwrap();
            let d = des_decrypt(&ct, &c.r2);
            for i in 0..8 {
                out.push(d[i] ^ prev[i]);
            }
            prev = ct;
    ...
    for i in 0..4 {
        let ct_le: Vec<Boolean<Fr>> = w.auth2_le[i * 64..(i + 1) * 64].to_vec();
        let ct_des = le_to_des(&ct_le);
        let ct_bytes: [u8; 8] = c.auth2[i * 8..(i + 1) * 8].try_into().unwrap();
        let raw = des_gadget(cs.clone(), &ct_des, &w.r2_le, false, &ct_bytes, &c.r2)?;
        let prev_des: Vec<Boolean<Fr>> = if i == 0 {
            vec![Boolean::FALSE; 64]
        } else {
            le_to_des(&w.auth2_le[(i - 1) * 64..i * 64].to_vec())
        };
...
/// Constraint 4 (circom `MacVerify13`): `MAC_verify_8(p, 0x13)`.
fn mac_verify(
    cs: ConstraintSystemRef<Fr>,
    pt_le: &[Boolean<Fr>],
    pt_native: &[u8],
...
impl ConstraintSynthesizer<Fr> for FelicaCircuit {
    fn generate_constraints(
        ...
        use crate::des::{des_decrypt, des_encrypt};
        let wires = constrain_public_packing(cs.clone(), &self)?;
        constrain_challenges(cs.clone(), &self, &wires)?;
        let (pt_le, pt_native) = cbc_decrypt_blocks(cs.clone(), &self, &wires)?;

        mac_verify(cs.clone(), &pt_le, &pt_native)?;

        // Constraint 5: p.tid == tail_6(r1): pt[2..8] vs r1[2..8].
        for i in 16..64 {
            pt_le[i].enforce_equal(&wires.r1_le[i])?;
        }
        // Constraint 6: p.idi == idi: pt[8..16] vs idi.
        for i in 0..64 {
            pt_le[64 + i].enforce_equal(&wires.idi_le[i])?;
        }
